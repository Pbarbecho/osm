...
import uuid
import click
from osm import campaign, build_executable, merge
from osm import custom_analyzer
...
def build(config, project_directory, ext, filename):
    ...
    if ext not in ['cc', 'ccp']:
        ext = 'cc'
    build_executable.project(project_directory, ext, filename)
...
def launcher(config, omnet_path, output_dir, max_processes, sim_time, repetitions, analyze, variables_path, inifile,
    ...
    campaign.run(output_dir, updated_max_processes, omnet_path, sim_time, repetitions, analyze, variables_path, inifile,
...
def parser(config, max_processes, input_dir, output_dir, output_filename, eval_path):
    ...
    if os.path.exists(input_dir) and os.listdir(input_dir):
        updated_max_processes = get_MAX_PROCESS(config, max_processes)
        # Parser
        merge.merge_files(input_dir, output_dir, output_filename, eval_path, updated_max_processes, config.iteration_variables_path)
    else:
...
def analyzer(config, input_cvs_file, output_dir, interactive_pivot_table):
    ...
    if os.path.exists(input_cvs_file):
        # Analyzer
        custom_analyzer.custom_filter_plots(input_cvs_file, output_dir, interactive_pivot_table, config.iteration_variables_path)
    else:
